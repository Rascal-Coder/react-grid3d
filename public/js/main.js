...
;(function(window) {
	...
	IsoGrid.prototype._init = function() {
		...
		window.imagesLoaded(this.gridEl, function() {
			// initialize masonry
			self.msnry = new window.Masonry(self.gridEl, {
			...
			if( self.options.type === 'scrollable' ) {
				new window.AnimOnScroll(self.gridEl, {
					minDuration : 1,
					maxDuration : 1.2,
					viewportFactor : 0
				});
			...
			window.classie.add(self.gridEl, 'grid--loaded');
		});
	};
	...
	IsoGrid.prototype._expandSubItems = function(item) {
		...
		subItems.forEach(function(subitem, pos) {
			window.dynamics.stop(subitem);
			window.dynamics.animate(subitem, self.options.stackItemsAnimation.properties(pos), self.options.stackItemsAnimation.options(pos, subItemsTotal));
		});
	};

	IsoGrid.prototype._collapseSubItems = function(item) {
		var itemLink = item.querySelector('a');
		[].slice.call(itemLink.querySelectorAll('.layer')).forEach(function(subitem) {
			window.dynamics.stop(subitem);
			window.dynamics.animate(subitem, {
